...
import controllers.SetIntoTheStore;
...
/**
 * Unit test for the SetIntoTheStore class.
 */
@RunWith(MockitoJUnitRunner.class)
public class SetIntoTheStoreTest {
	...
	MockSetIntoTheStore controllerServlet;
	
	
	@Before
	public void setup() throws IOException, ServletException {
		
		controllerServlet = spy(new MockSetIntoTheStore(queueModule, resModule));
	...
	@Test
	public void queueFull() throws ServletException, IOException, CLupException {
		...
		when(queueModule.setIntoTheStore(anyInt())).thenReturn(false);
		when(req.getParameter("reservationId")).thenReturn("100");
		when(req.getParameter("groceryId")).thenReturn("123");
		...
		verify(req, times(2)).getParameter(anyString());
		verify(res).sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "The store is full");
	}
	...
	public void setIntoOk() throws ServletException, IOException, CLupException {
		...
		when(resModule.getReservation(anyInt())).thenReturn(reservation);
		when(queueModule.setIntoTheStore(anyInt())).thenReturn(true);
		when(req.getParameter("reservationId")).thenReturn("100");
		when(req.getParameter("groceryId")).thenReturn("123");
	...
	class MockSetIntoTheStore extends SetIntoTheStore {

		private static final long serialVersionUID = 1L;

		public MockSetIntoTheStore(QueueUpdateManagementImplementation queueModule, ReservationHandlerImplementation resModule) {
